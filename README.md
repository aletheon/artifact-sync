...
[Root Folder] (Default: "Artifact Sync")
  └── [AI Provider] (e.g. "Gemini", "ChatGPT")
      └── [Conversation Title]
            ├── artifacts/        <-- Captured images/diagrams from the AI
            ├── attachments/      <-- Files you uploaded to the AI
            ├── pdfs/             <-- (If PDF mode enabled)
            └── My_Prompt_123.md  <-- The conversation log
```
...
*   **Conversation Log**:
    *   **Markdown Mode (Default)**: Creates a new `.md` file for every turn (or appends logic depending on settings) directly in the conversation folder.
    *   **PDF Mode**: Generates a PDF document stored in the `pdfs/` subfolder.
